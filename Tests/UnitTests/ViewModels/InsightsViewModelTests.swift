...
final class MockInsightFlaggingService: InsightFlaggingService {
    private var _insights: [Insight] = []
    private var _flaggingStatus: FlaggingStatus = .idle
    ...
    override var insights: [Insight] {
        _insights
    }

    override var flaggingStatus: FlaggingStatus {
        _flaggingStatus
    }
    ...
    func simulateInsights(_ insights: [Insight]) {
        _insights = insights
        // Trigger publisher update
        objectWillChange.send()
    }

    func simulateFlaggingStatus(_ status: FlaggingStatus) {
        _flaggingStatus = status
        objectWillChange.send()
    }
    ...
    override func removeInsight(_ insight: Insight) {
        ...
        _insights.removeAll { $0.id == insight.id }
        objectWillChange.send()
    }
    ...
    override func clearAutoGeneratedInsights() {
        ...
        _insights.removeAll { $0.isAIGenerated }
        objectWillChange.send()
    }
}
